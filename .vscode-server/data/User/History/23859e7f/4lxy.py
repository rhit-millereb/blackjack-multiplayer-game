...
class Table:
    ...
    def userMove(self, user: User, hand: Hand, move: str, server: Server) -> bool:
        if (move == "hit" or move == "double"):
            ...
            try:
                server.sendMessageToAllUsers("Table||"+user.data.name+"||"+hand.toString(True)+" ("+hand.result2String()+")")
            ...
            if (len(hand.cards) != 2):
                print("Invalid number of cards, move not allowed")
                server.sendMessageToUser(user.data.name, "Message||Invalid number of cards, move not allowed")
                return True
            if (hand.cards[0].rank != hand.cards[1].rank):
                print("Cards must be of same rank")
                server.sendMessageToUser(user.data.name, "Message||Cards must be of same rank")
                return True
            ...
            server.sendMessageToUser(user.data.name, "Hand split.\n"+
                                     "Original hand now: "+hand.toString(True)+" ("+hand.result2String()+")\n"+
                                     "Second hand is: "+newHand.toString(True)+" ("+newHand.result2String()+")\n")
            
            try:
                server.sendMessageToAllUsers("Message||"+user.data.name+" split hand.")
                server.sendMessageToAllUsers("Table||"+user.data.name)
            except Exception as e:
                print(str(e)+"\n(Surpressed)")
            ...
            print("Invalid move")
            server.sendMessageToUser(user.data.name, "Invalid move")
            return True
