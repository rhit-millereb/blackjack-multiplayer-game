...
class Table:
    ...
    def drawHands(self, server: Server):
        ...
        if not self.stack.canDraw(6+(6*len(self.users))):
            ...
            server.sendMessageToAllUsers("Message||Stack Out: Running new shuffle algorithm...")
            time.sleep(3)
            self.stack.clear()
            self.stack.create(NUMDECKS)
        ...
        if (move == "hit" or move == "double"):
            ...
            if (move == "double"):
                ...
                if user.data.bank - hand.bet < 0:
                    ...
                    server.sendMessageToUser(user.data.name, "Message||Move not allowed, not enough money")
                    time.sleep(1)
                    return True
            ...
            if user.data.bank - hand.bet < 0:
                ...
                server.sendMessageToUser(user.data.name, "Message||Split not allowed, not enough money")
                time.sleep(1)
                return True
