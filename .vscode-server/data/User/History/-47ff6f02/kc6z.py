...
from user import USERFILESPATH
from auth import ClientAuth
from chat import Chat
import time
from leaderboard import LeaderboardServer
...
class Client:
    ...
    def __init__(self, name: str, ip: str, s: socket, clientID: str):
        self.name = name
        self.ip = ip
        self.s = s
...
class Server:
    ...
    def __init__(self, settings: Settings):
        ...
        # init the chat server
        self.chatServer = Chat(self.authServer.clearedUsers, self.clients)

        # init the leaderboard server
        self.leadServer = LeaderboardServer(self.authServer.clearedUsers)
    ...
    def __signUserIn(self, client_s, address):
        ...
        try:
            ...
            print("Error: Unable to parse client request")
            return
        ...
        # ensure there are six items in the connection packet
        if len(info) != 6:
        ...
        # ensure there is room for client to connect
        if len(self.clients) > self.settings.maxConnectedUsers:
            print("Max number of connected users reached")
            client_s.send("Error||Max number of users connected, try connecting later".encode())
            client_s.close()
            return

        clientID = info[4]

        cleared = self.authServer.userCleared(clientID)
        count = 0
        while(not cleared):
            print("User not cleared: "+str(address))
            if count > 10:
                print("User never cleared, removing.")
                client_s.send("Error||Could not authenticate user".encode())
                client_s.close()
                return
        ...
        new_client = Client(username, address, client_s ,clientID)
        ...
        if create_account == "sign in":
            ...
            if not self.settings.allowNewUsers:
                ...
                client_s.close()
                return
            if len([name for name in os.listdir(USERFILESPATH) if os.path.isfile(name)]) > self.settings.maxAccounts+2:
                new_client.sendMessage("Error||Max accounts reached, cannot create more")
                client_s.close()
                return
