...
class Table:
    ...
    # returns true if user can make another move
    def userMove(self, user: int, move: str) -> bool:
        if (move == "hit"):
            # add a card to the user
            self.hitUser(user)
            
            print(self.users[user].name+": "+str(self.users[user].toString(True)))

            # get new result
            result = self.users[user].getResult()

            print(self.users[user].name+" now has: "+str(self.users[user].result2String(result)))

            if (self.users[user].totalCards()[0] < 21):
                return True
            
            return False
        elif (move == "stand"):
            print(self.users[user].name+" stands.")
            return False
        elif (move == "double"):
            self.hitUser(user)
            return False
            ...
            raise Exception("Invalid Move: Move not among valid")

    def hitUser(self, user: int):
        self.users[user].addCard(self.stack.drawCard())
    ...
    def doesDealerHit(self) -> bool:
        ...
        if (len(total) > 1):
            # dealer has ace, no hit on soft 17
            if (total[1] < 17):
                return True
            return False
        else:
