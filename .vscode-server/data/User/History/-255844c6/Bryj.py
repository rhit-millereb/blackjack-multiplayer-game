        ...
        while(True):
            if self.state == SETUP:
                ...
                for user in self.table.users:
                    print("Awaiting bet from "+user.name)
                    bet = int(input())

                    # CHECK IF BET POSSIBLE HERE
                ...
                print(self.table.toString(True))

                # total all of the dealers cards
                self.dealerTotal = self.table.dealer.totalCards()
                print("Dealer has: "+self.table.dealer.result2String(self.table.dealer.getResult(self.dealerTotal)))

                # total all of the user's cards
                i = 0
                for user in self.table.users:
                    self.userTotals[i] = user.totalCards()
                    print(user.name+" has: "+str(user.result2String(user.getResult(self.userTotals[i]))))
