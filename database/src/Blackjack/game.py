...
class Game:
    ...
    def begin(self):
        ...
        if self.table is None:
            raise Exception("Cannot Start: Table must be assigned")
        ...
        while(True):
            ...
            for user in self.table.users:
                ...
                for hand in user.hands:
                    ...
                    if bet is None or bet == "" or bet == "None":
                        try:
                            self.server.sendMessageToUser(user.data.name, "Message||Leaving game...")
                        except Exception as e:
                            print("Error: Unable to send quit message. User likely timed out...")
                        usersToRemove.append(user)
                        break
                    ...
                    if bet > user.data.bank:
                        self.server.sendMessageToUser(user.data.name, "Error||Fat Ass")
                        usersToRemove.append(user)
                        break
            ...
            for user in usersToRemove:
                ...
                try:
                    ...
                    print("Couldn't quit: "+user.data.name)
                    
                try:
                    self.server.clients.remove(self.server.getClientFromName(user.data.name))
                except Exception as e:
                    print("Couldn't remove: "+user.data.name)
            ...
            if (self.table.dealer.getResult(self.dealerTotal) == BLACKJACK):
                ...
                # iterate over all users and hands to catch if user also got black jack
                for user in self.table.users:
                    for hand in user.hands:
                        if hand.getResult() == BLACKJACK:
            ...
            for user in self.table.users:
                ...
                for hand in user.hands:
                    ...
                    if hand.getResult() == BLACKJACK:
                        self.server.sendMessageToAllUsers("Message||"+user.data.name+" has BLACKJACK!")
                        time.sleep(2)
                        continue
                    
                    # notify all other users who is waiting for a move
                    self.server.sendMessageToAllExcept("Message||Waiting for move from "+user.data.name, user.data.name)
            ...
            for user in self.table.users:
                ...
                for hand in user.hands:
                    ...
                    if (total < dealerTotal or total == 0):
                        print(user.data.name+" hand "+str(i+1)+" loses")

                        text += "Hand "+str(i+1)+" loses~"

                        user.data.losses += 1
                    elif (total == dealerTotal and hand.getResult() != BLACKJACK):
                        ...
                        if not hand.getResult() == BLACKJACK:
                            # add bets to user bank
                            user.data.bank += 2 * hand.bet
                        else:
                            user.data.bank += int((3/2)*hand.bet)
                ...
                # send updated bank to the client
                if not self.server.sendMessageToUser(user.data.name, "Bank||"+str(user.data.bank)):
                    usersToRemove.append(user)
                    break
            ...
            self.server.leadServer.calculateAllData()
